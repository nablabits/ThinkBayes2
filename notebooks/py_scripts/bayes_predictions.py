...
class BayesPredictions:
    def __init__(self, initial_history=4, risk=1):
        ...
        self.price_range = np.linspace(
            10_000, 80_000, 1401
        )  # min showcase 18.3k, max 71.6k
        self.off_ranges = self.get_off_ranges()

        self.outcomes = pd.DataFrame(
            {"p1_wins": [0.0], "p2_wins": [0.0], "no_one_wins": [0.0]}
        )
    ...
    def bayes_estimator(self):
        ...
        likelihood = norm(self.current_bid, self.error_history.std()).pdf(
            self.price_range
        )
    ...
    def estimate_winning_bid(self, player):
        ...
        chances_ranges = np.apply_along_axis(
            self.get_chances_of_winning, 1, self.off_ranges
        )
    ...
    def compute_outcome(self):
        ...
        p1_wins = (
            (p1_underbids_by_less | p2_overbids) & ~p1_overbids
        ).mean()  # 54% * 76% = 41%
        p2_wins = (
            (p2_underbids_by_less | p1_overbids) & ~p2_overbids
        ).mean()  # 52% * 72% = 37%
...
class MakeUpP1(BayesPredictions):
    ...
    def create_base_df(self):
        ...
        df.loc[:, "bid_1"] = np.random.normal(
            df.showcase_1 * self.worsen_factor, df.diff_1.std()
        )
    ...
    def save_data_to_file(self):
        ...
        self.outcomes.to_csv(
            f"09-outcomes/outcomes-{self.initial_history}-{f}-worse.csv"
        )
